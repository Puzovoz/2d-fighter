...
// Window implementation of the render manager.
...
func (wm *WindowManager) Loop(ch chan Context) {
	...
	for ctx := range ch {
		...
		if float32(timeBetweenFrames) < desiredDelta {
			timeToSleep := time.Duration(uint64(desiredDelta) - timeBetweenFrames)
...
// Update the game screen from given render context.
